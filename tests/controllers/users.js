...
test('Get 200 on user request', async t => {
  ...
  const res = await app.get(`/users/${users[0]._id}`);
...
test('Get 200 and added user on add user', async t => {
  ...
  const res = await app.put('/users/').send(NEW_USER);
...
test('Get 406 if user with the same login exists', async t => {
  ...
  const res = await app.put('/users/').send({ login: users[0].login, password: 'FAKE_PASSWORD' });
...
test('Get 202 on successfull deletion', async t => {
  ...
  const res = await app.delete(`/users/${users[0]._id}`);

  t.is(res.status, 202);
});

test('Get 405 when there is no id parameter', async t => {
  const res = await app.delete('/users/');
